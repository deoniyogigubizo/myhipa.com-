 ...
 */
export async function getPricingSuggestion(productId: ObjectId): Promise<IPricingSuggestion | null> {
  await connectDB();
...
async function findComparableProducts(
  ...
  if (product.embeddingVector) {
    return candidates
      ...
      .map(c => ({
        ...c,
        similarity: cosineSimilarity(product.embeddingVector!, c.embeddingVector!)
      }))
  ...
  const productWords = new Set((product.title as string).toLowerCase().split(' '));
  return candidates
    .map(c => {
      const candidateWords = new Set((c.title as string).toLowerCase().split(' '));
      const intersection = ([...productWords] as string[]).filter(w => candidateWords.has(w));
...
async function getCategoryDemandSignal(
  ...
  const change = previousSearchCount > 0 
...
export async function getDemandForecast(
  ...
  const predictedDemand = Math.round(avgCount * (1 + (trend === 'rising' ? 0.1 : trend === 'falling' ? -0.1 : 0)));
