...
import { generateTextEmbedding, generateImageEmbedding, cosineSimilarity } from './embedding';
import type { 
  IQueryUnderstanding, 
  ISearchResult,
  IVisualSearchResult 
...
export async function unifiedSearch(
  ...
  if (options.useSemantic || keywordResults.length < 10 || isComplexQuery(query)) {
    semanticResults = await semanticSearch(query, {
      category: options.category || queryUnderstanding.filters.category || undefined,
      minPrice: queryUnderstanding.filters.minPrice,
      maxPrice: queryUnderstanding.filters.maxPrice,
      brand: queryUnderstanding.filters.brand,
  ...
  // Blend results
  const blendedResults = blendSearchResults(keywordResults, semanticResults, keywordResults.length >= 10);
...
function isComplexQuery(query: string): boolean {
  const complexIndicators = [
    'under', 'around', 'between', 'cheap', 'affordable', 'best',
    'good', 'with', 'for', 'recommend', 'similar', 'like'
  ];
...
function blendSearchResults(
  ...
  const results = Array.from(itemMap.values()).map(r => ({
    ...r,
    finalScore: r.keywordScore + r.semanticScore + r.sellerTrustScore + r.adBoostScore
  }));
...
async function enrichWithSellerTrust(results: ISearchResult[]): Promise<void> {
  ...
  const sellerTrustMap = new Map(
    sellers.map(s => [
      s._id.toString(),
      Math.min(1, 0.3 + (s.stats?.avgRating || 0) / 10 + (s.kycStatus === 'verified' ? 0.2 : 0))
    ])
  );
  ...
  for (const product of products) {
    ...
    if (result) {
      result.sellerTrustScore = sellerTrustMap.get(product.sellerId.toString()) || 0.3;
