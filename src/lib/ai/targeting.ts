 ...
 */
export async function buildUserInterestSegments(
  userId: ObjectId,
): Promise<string[]> {
  await connectDB();
...
export function projectDailySpend(
  ...
  const expectedSoFar = todayHourlyWeights.reduce(
    (sum, t) => sum + t.weight,
    0,
  );

  // Current actual spend
  const actualSoFar = hourlySpend
  ...
  if (actualSoFar > expectedSoFar * 1.2) {
    suggestions.push(
      "Campaign is spending faster than expected. Consider reducing bid to stretch budget.",
    );
  } else if (actualSoFar < expectedSoFar * 0.5 && expectedSoFar > 0) {
    suggestions.push(
      "Campaign is underspending. Consider increasing bid or reviewing targeting.",
    );
  }
