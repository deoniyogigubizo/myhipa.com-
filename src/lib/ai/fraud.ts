 ...
 */
export async function analyzeReviewForFraud(reviewId: ObjectId): Promise<IFakeReviewSignal> {
  await connectDB();
  ...
  if (review.createdAt && review.orderId) {
    const order = await Order.findById(review.orderId).select('createdAt').lean();
    if (order) {
      const minutesDiff = (review.createdAt.getTime() - order.createdAt.getTime()) / (1000 * 60);
  ...
  const genericPhrases = [
    'good', 'great', 'nice', 'ok', 'okay', 'not bad',
    'product is good', 'very good', 'awesome'
  ];
  const reviewText = review.body?.toLowerCase() || '';
  const isGeneric = genericPhrases.some(phrase => reviewText.includes(phrase));
  signals.genericVagueText = isGeneric && reviewText.length < 50;

  // Check 3: Reviewer account created same day
  const reviewer = await User.findById(review.buyerId).select('createdAt').lean();
  ...
  signals.suspiciousPattern = 
    signals.postedWithinMinutesOfPurchase && 
    signals.genericVagueText;
...
export async function flagSuspiciousReview(
  ...
  const severity = signal.confidence >= 0.8 ? 'critical' 
    : signal.confidence >= 0.6 ? 'high' 
    : signal.confidence >= 0.4 ? 'medium' 
    : 'low';
  ...
  await FraudSignal.create({
    ...
    recommendedAction: signal.confidence >= 0.8 ? 'ban' 
      : signal.confidence >= 0.6 ? 'suspend' 
      : signal.confidence >= 0.4 ? 'flag' 
      : 'watch',
    createdAt: new Date()
  });
}
 ...
 */
export async function analyzeDisputeAbuse(userId: ObjectId): Promise<IDisputeAbuseSignal> {
  await connectDB();
 ...
 */
export async function analyzeSellerFraud(sellerId: ObjectId): Promise<ISellerFraudSignal> {
  await connectDB();
  ...
  const { Transaction } = await import('@/lib/database/schemas/transaction.schema');
  ...
  const totalPayout = recentPayouts.reduce((sum, t) => sum + (t.sellerPayout || 0), 0);
...
export async function detectAccountTakeover(
  ...
  const knownDevices = new Set(recentLogins.map(l => (l as unknown as { metadata?: { deviceId?: string } })?.metadata?.deviceId).filter(Boolean));
  const knownIPs = new Set(recentLogins.map(l => (l as unknown as { metadata?: { ipAddress?: string } })?.metadata?.ipAddress).filter(Boolean));

  // Check for new device
  const newDevice = loginData.deviceId ? !knownDevices.has(loginData.deviceId) : false;
