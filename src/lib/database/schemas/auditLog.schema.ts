...
AuditLogSchema.statics.findByIP = function (ip: string, limit: number = 100) {
  return this.find({ "actor.ip": ip }).sort({ createdAt: -1 }).limit(limit);
};

/**
 * Get recent admin actions
 */
AuditLogSchema.statics.getAdminActions = function (
  days: number = 7,
  limit: number = 100,
) {
...
AuditLogSchema.statics.search = function (
  ...
  if (filters.userId)
    query["actor.userId"] = new mongoose.Types.ObjectId(filters.userId);
