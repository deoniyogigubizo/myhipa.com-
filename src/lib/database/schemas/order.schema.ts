...
OrderSchema.index({ sellerId: 1, status: 1, createdAt: -1 }, { name: 'seller_status_date_idx' });
...
OrderSchema.virtual('canDispute').get(function() {
  return this.status === 'dispute_window' && 
 ...
 */
OrderSchema.statics.findByBuyer = function(buyerId: mongoose.Types.ObjectId | string) {
 ...
 */
OrderSchema.statics.findBySeller = function(sellerId: mongoose.Types.ObjectId | string) {
 ...
 * Mark as paid
 */
OrderSchema.methods.markAsPaid = function(paymentMethod: string, provider?: string, gatewayRef?: string) {
  this.payment = {
 ...
 * Mark as shipped
 */
OrderSchema.methods.markAsShipped = function(trackingNumber?: string, courier?: string) {
