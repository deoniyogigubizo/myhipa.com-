...
SellerSchema.index(
  { "store.slug": 1 },
  { unique: true, name: "store_slug_unique" },
);

// Geo index for location queries
SellerSchema.index(
  { "store.location.coords": "2dsphere" },
 ...
 */
SellerSchema.statics.findByUserId = function (
 ...
 */
SellerSchema.statics.findNearby = function (
 ...
 * Update statistics
 */
SellerSchema.methods.updateStats = function (
  updates: Partial<ISeller["stats"]>,
) {
 ...
 * Add to wallet
 */
SellerSchema.methods.addToWallet = function (
  amount: number,
  type: "available" | "pending" | "held",
) {
 ...
 * Deduct from wallet
 */
SellerSchema.methods.deductFromWallet = async function (
  amount: number,
): Promise<boolean> {
 ...
 * Add payout method
 */
SellerSchema.methods.addPayoutMethod = function (
  method: ISeller["payoutMethods"][0],
) {
