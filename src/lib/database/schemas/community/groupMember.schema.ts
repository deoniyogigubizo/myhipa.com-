...
GroupMemberSchema.index({ groupId: 1, userId: 1 }, { unique: true, name: 'group_user_unique' });
 ...
 * Find membership
 */
GroupMemberSchema.statics.findMembership = function(groupId: string, userId: string) {
  return this.findOne({ 
 ...
 * Find group members
 */
GroupMemberSchema.statics.findByGroup = function(groupId: string, options?: { role?: string; limit?: number }) {
 ...
 */
GroupMemberSchema.statics.isMember = async function(groupId: string, userId: string): Promise<boolean> {
 ...
 * Get member count
 */
GroupMemberSchema.statics.getMemberCount = async function(groupId: string): Promise<number> {
