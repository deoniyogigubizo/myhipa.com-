    ...
    path: [
      {
        type: String,
        description:
          'Full category path e.g., ["electronics", "smartphones", "android"]',
      },
    ],
  },
  { _id: false },
...
const ProductVariantSchema = new Schema(
  {
    ...
    attributes: {
      ...
      of: String,
      description:
        'Variant attributes e.g., { color: "Black", storage: "128GB" }',
    },
    price: {
      type: Number,
      required: true,
...
ProductSchema.virtual("inStock").get(function () {
  ...
  return (
    (this.inventory?.totalStock || 0) > 0 || this.inventory?.allowBackorder
  );
});
 ...
 */
ProductSchema.statics.findInPriceRange = function (
  minPrice: number,
  maxPrice: number,
) {
  return this.find({
 ...
 */
ProductSchema.statics.findSimilar = function (
  embedding: number[],
  limit: number = 10,
) {
  return this.find({
 ...
 */
ProductSchema.statics.findByTags = function (
  tags: string[],
  matchAll: boolean = false,
) {
  const query = matchAll ? { $all: tags } : { $in: tags };
...
ProductSchema.methods.updateStock = function (
  ...
  if (variant) {
    ...
    this.inventory.totalStock = this.variants.reduce(
      (sum, v) => sum + v.stock,
      0,
    );
  }
 ...
 */
ProductSchema.methods.getPriceForQuantity = function (
  quantity: number,
): number {
