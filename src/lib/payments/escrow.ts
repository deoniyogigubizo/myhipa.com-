...
export class EscrowService {
  private _escrowQueue: Queue | null = null;
  ...
  private get escrowQueue(): Queue {
    if (!this._escrowQueue) {
      this._escrowQueue = new Queue("escrow", {
        connection: {
  ...
  async holdPayment(orderId: string, gatewayTransactionId: string) {
    ...
    if (verification.amount !== order.pricing.total) {
      console.error(
        `Amount mismatch: expected ${order.pricing.total}, got ${verification.amount}`,
      );
    ...
    this.blockchain
      ...
      .then((hash) => {
        Transaction.findOneAndUpdate(
          { orderId },
          { "blockchain.createTxHash": hash },
        );
      })
      .catch((err) =>
        console.error("Blockchain record failed (non-critical):", err),
      );
  ...
  async markDelivered(orderId: string) {
    ...
    await Order.findByIdAndUpdate(orderId, {
      ...
      $push: {
        statusHistory: { status: EscrowStatus.DISPUTE_WINDOW, at: now },
      },
    });
  ...
  async releaseFunds(
    ...
    this.blockchain
      ...
      .then((hash) => {
        Transaction.findOneAndUpdate(
          { orderId },
          { "blockchain.releaseTxHash": hash },
        );
      })
      .catch((err) => console.error("Blockchain release record failed:", err));

    console.log(
      `Escrow released: order=${orderId} type=${releaseType} amount=${tx.sellerPayout}`,
    );
  }
  ...
  async adminResolve(
    ...
    const buyerRefund = Math.round(
      tx.amount * (resolution.buyerRefundPct / 100),
    );
    ...
    // Process payouts
    if (sellerReceives > 0) {
      await Seller.findByIdAndUpdate((await Order.findById(orderId)).sellerId, {
        $inc: {
          "wallet.available": sellerReceives,
          "wallet.pending": -sellerReceives,
        },
      });
    }

    if (buyerRefund > 0) {
      await User.findByIdAndUpdate((await Order.findById(orderId)).buyerId, {
