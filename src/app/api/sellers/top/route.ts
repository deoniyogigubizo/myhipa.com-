...
export async function GET(request: Request) {
  try {
    ...
    const allSellers = await db.collection('sellers')
    ...
    const shuffled = [...allSellers].sort((a, b) => {
      ...
      const aScore = (a.stats?.totalRevenue || 0) + (a.stats?.avgRating || 0) * 1000;
      const bScore = (b.stats?.totalRevenue || 0) + (b.stats?.avgRating || 0) * 1000;
