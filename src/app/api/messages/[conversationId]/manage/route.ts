...
export async function POST(
  ...
  try {
    ...
    const messageObjectIds = messageIds.map(
    ...
    switch (action) {
      ...
      case "delete_for_everyone": {
        ...
        if (messages.length !== messageIds.length) {
          return NextResponse.json(
            {
              success: false,
              error: "Can only delete your own messages within 48 hours",
            },
            { status: 400 },
          );
        }
      ...
      case "star": {
        ...
        for (const message of messages) {
          ...
          const isStarred = starredBy.some(
            (id: mongoose.Types.ObjectId) => id.toString() === user.userId,
          );
