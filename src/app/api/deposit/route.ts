...
export async function POST(request: NextRequest) {
  try {
    ...
    // Initialize MTN service inside the handler to avoid build-time initialization errors
    ...
    if (!amount || amount < 1000) {
      return NextResponse.json(
        { error: "Minimum deposit amount is 1,000 RWF" },
        { status: 400 },
      );
    }
    ...
    if (!user.phone) {
      return NextResponse.json(
        { error: "Phone number required for MTN MoMo deposit" },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json({
      message:
        "Payment initiated successfully. Please check your MTN MoMo phone for the payment prompt.",
      txRef: payment.txRef,
      amount,
      currency: "RWF",
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("Failed to create access token: 401")) {
        return NextResponse.json(
          {
            error:
              "Payment service authentication failed. Please check MTN MoMo API credentials.",
          },
          { status: 500 },
        );
      }
      if (error.message.includes("Account holder is not active")) {
        return NextResponse.json(
          {
            error:
              "Your MTN Mobile Money account is not active. Please check your account status.",
          },
          { status: 400 },
        );
      }
      if (error.message.includes("Payment request failed")) {
        return NextResponse.json(
          {
            error:
              "Payment request failed. Please try again or contact support.",
          },
          { status: 400 },
        );
      }
