...
const NAV_ITEMS = [
  {
    ...
    icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none">
        <path
          d="M3 13h1v7c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2v-7h1a1 1 0 00.7-1.7l-9-9a1 1 0 00-1.4 0l-9 9A1 1 0 003 13zm7 7v-5h4v5h-4zm2-15.6l7 7V20h-3v-5c0-1.1-.9-2-2-2h-4c-1.1 0-2 .9-2 2v5H5v-8.6l7-7z"
          fill="currentColor"
        />
      </svg>
    ),
...
export default function SellerDashboard() {
  ...
  const [followedSellers, setFollowedSellers] = useState<Set<string>>(
    new Set(),
  );
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch(
        "/api/seller/products?limit=8&status=active",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    <div className="min-h-screen bg-gray-100">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span
                  className={`px-2 py-0.5 rounded text-xs font-medium ${
                    data.seller.tier === "gold"
                      ? "bg-yellow-100 text-yellow-700"
                      : data.seller.tier === "silver"
                        ? "bg-gray-100 text-gray-600"
                        : data.seller.tier === "pro"
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          ></div>
          <aside className="absolute left-0 top-0 bottom-0 w-72 bg-white animate-slide-in overflow-y-auto">
      ...
      <div className="flex pt-14 md:pt-0">
        ...
        <main className="flex-1 p-4 md:p-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6">
            {/* Revenue Card - Wider on mobile */}
            <div className="col-span-1 md:col-span-1 lg:col-span-1 bg-white rounded-lg p-3 md:p-5 border border-gray-200">
              ...
              {data.kpis.revenue.change && (
                <div
                  className={`text-xs mt-2 ${
                    ...
                    data.kpis.revenue.change.startsWith("+")
                      ? "text-green-600"
                      : "text-red-600"
                  }`}
                >
                  {typeof data.kpis.revenue.change === "string"
                    ? data.kpis.revenue.change
                    : `${data.kpis.revenue.change > 0 ? "+" : ""}${data.kpis.revenue.change}%`}{" "}
                  vs last month
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-lg p-3 md:p-5 border border-gray-200">
              ...
              <div className="text-xs text-gray-400 mt-2">
                {data.kpis.orders.processing} processing ·{" "}
                {data.kpis.orders.shipped} shipped ·{" "}
                {data.kpis.orders.delivered} delivered
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg p-3 md:p-5 border border-gray-200">
              ...
              {data.kpis.escrow.releasingToday > 0 && (
                <div className="text-xs text-green-600 mt-2">
                  {formatCurrency(data.kpis.escrow.releasingToday)} releasing
                  today
                </div>
              )}
            </div>

            {/* Rating Card - Wider on mobile */}
            <div className="col-span-1 md:col-span-1 lg:col-span-1 bg-white rounded-lg p-3 md:p-5 border border-gray-200">
              <div className="flex items-center justify-between mb-3">
                <div className="w-10 h-10 bg-gray-100 rounded flex items-center justify-center">
                  <svg
          ...
          {/* Your Products Grid */}
          <div className="mt-4 md:mt-6 bg-white rounded-lg border border-gray-200 p-3 md:p-5">
